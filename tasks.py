...
def all(ctx):
    ...
    shutil.copy2(
        ROOT / "blueprint" / "print" / "print.pdf", ROOT / "docs" / "blueprint.pdf"
    )
...
def check(ctx):
    ...
    if not DECLS_FILE.exists():
        print(
            "[ERROR] Declarations file not found. Please run `lake -Kenv=dev build %s:docs` first."
        )
        exit(1)
    ...
    if not DEP_GRAPH_FILE.exists():
        print(
            "[ERROR] Dependency graph file not found. Please run `inv all` (or only `inv web`) first."
        )
        exit(1)

...
class Checker:
    def __init__(self, nperms=3, binTime=60, approxTime=5, mode="skipApprox",
    ...
    def check(self, ground, test, instanceName):
        ...
        command = ['lake', 'env', 'lean','--run', tmpFile, instanceName, self.mode, str(self.nPermutations), str(self.equivSolverTime), str(self.approxSolverTime), outputJsonFile]
        process = Popen(command, stdin=PIPE, stdout=PIPE, cwd=ROOT_DIR, preexec_fn=os.setsid)
        try:
            process.communicate()

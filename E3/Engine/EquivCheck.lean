...
/-- Modified version of `evalSmt`used by E3. Similar to the normal usage in `euclid_*` tactics, except we include different -/
...
def solvePerms
  ...
  for perm in perms do
    ...
    if fwdLHS.success && bwdLHS.success then
       logInfo "LHS proved equivalent; preconditions will be included for RHS"
       assumptions := (← getGroundLHS).splitConjuncts
    logInfo "checking RHS ..."

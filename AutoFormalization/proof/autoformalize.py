...
def preceding_propositions(idx):
    with open("book_propositions.json", 'r', encoding='utf-8') as f:
...
def examples(dataset, category, num, reasoning):
    ...
    for idx in indices:
        if dataset == 'UniGeo':
            ...
            informal_proof_path = os.path.join(EXAMPLE_DIR, dataset, category, 'proofs', f'{idx}.txt')
            with open(informal_proof_path) as f:
                informal_proof = f.read().rstrip('\n')
                informal_proof = re.sub(r'\n', ', ', informal_proof) + '.'
        else:
            text_proof_path = os.path.join(EXAMPLE_DIR, dataset, category, 'texts_proofs', f'{idx}.txt')
        ...
        if dataset == 'UniGeo':
            ...
            content.append({"type": "text", "text": f'English Statement and Proof: {text_proof}\nFormalized Statement: {formal_statement}\nFormalized Proof: <<< {formal_proof} >>>\n'})
...
def main():
    ...
    with open(f'{args.dataset.lower()}_instruction.txt') as f:
    ...
    for c in args.category:
        ...
        for i in tqdm.tqdm(testing_idx):
            ...
            if args.dataset == 'UniGeo':
                ...
                informal_proof_path = os.path.join(ROOT_DIR, args.dataset, c, 'proofs', f'{i}.txt')
                with open(informal_proof_path) as f:
                    informal_proof = f.read().rstrip('\n')
                    informal_proof = re.sub(r'\n', ', ', informal_proof) + '.'
            else:
                instruction = instruction.replace(r"{PRECEDING_PROPOSITIONS}", preceding_propositions(i))
                text_proof_path = os.path.join(ROOT_DIR, args.dataset, c, 'texts_proofs', f'{i}.txt')
                with open(text_proof_path) as f:
                    text_proof = f.read()
                ...
                if response:
                    ...
                    if match:
                        ...
                        with open(result_file, 'w') as f:
                            lean_file = format_content(args.dataset, namespace, f"formalized_theorem_{i}", formal_statement, pred)
                            f.write(lean_file)
                        break
                    else:
